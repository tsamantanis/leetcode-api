...
describe('User API endpoints', () => {
  ...
  after((done) => {
    User.deleteMany({
      email: ['myuser@mailinator.com', 'anotheruser@mailinator.com'],
    }).then(() => {
      done();
    });
  });
  ...
  it('should post a new user', (done) => {
    chai
      ...
      .post('/users/register')
      .send({
        user: { email: 'anotheruser@mailinator.com', password: 'mypassword' },
      })
      .end((err, res) => {
        if (err) {
          done(err);
